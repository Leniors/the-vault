...
export const HoverEffect = ({
  ...
  return (
    <div
      className={cn(
        "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 py-10 bg-black",
        className
      )}
    >
      {items.map((item, idx) => {
  ...
  item.stock === 0 ? "bg-gray-600 cursor-not-allowed" : ""
                ...
                {item.stock !== undefined && item.stock > 5 && (
                  <div className="absolute bottom-5 right-3 bg-green-600 text-white text-xs px-2 py-0.5 rounded">
                    In stock
                  </div>
                )}
                {item.stock !== undefined &&
                  item.stock < 5 &&
                  item.stock > 0 && (
                    <div className="absolute bottom-5 right-3 bg-red-600 text-white text-xs px-2 py-0.5 rounded">
                      Low stock
                    </div>
                  )}
                {item.stock === 0 && (
